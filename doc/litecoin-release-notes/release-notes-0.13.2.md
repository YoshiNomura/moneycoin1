Moneycoincoin Core version 0.13.2 is now available from:
...
ECDSA signatures inside Moneycoincoin transactions now use validation using
...
Previous versions of Moneycoincoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Moneycoincoin Core was to raise the default minimum
relay fee.

Moneycoincoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Moneycoincoin Core 0.13.2 also introduces new default policy limits on the length and
...
Moneycoincoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Moneycoincoin Core will relay transactions with
...
entirely in Moneycoincoin Core 0.13, and it is at this time undecided whether the
...
Moneycoincoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Moneycoincoin Core automatically creates a hidden service to listen on, without
manual configuration. Moneycoincoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Moneycoincoin Core is listening, and
...
Moneycoincoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Moneycoincoin
Core to use floating fees. By default, Moneycoincoin Core will use floating
fees.
...
At all times, Moneycoincoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) MNC.
Furthermore, Moneycoincoin Core will never create transactions paying less than
...
Various code modernizations have been done. The Moneycoincoin Core code base has
...
for a new opcode in the Moneycoincoin scripting system that in combination with
...
Moneycoincoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Moneycoincoin Core.
...
The two phases are distinct in the Moneycoincoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Moneycoincoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Moneycoincoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Moneycoincoin block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** Moneycoincoin addresses (both P2PKH addresses
  ...
  believed to be provided by Moneycoincoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Moneycoincoin's current ledger (called the UTXO set),
  ...
  note that Moneycoincoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Moneycoincoin users to individually opt-in to almost any change in the Moneycoincoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Moneycoincoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Moneycoincoin
client to Moneycoincoin Core 0.13.2 to signal for segwit support. When a
...
Since Moneycoincoin Core 0.10.0, nodes have defaulted to only relaying and
